  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const lines = [
        ...
        `Выручка/мес: ${data.mrr}`,
        `База лидов: ${data.leads}`,
        `Платформа: ${data.platform}`,
        `Куда прислать: ${data.contact_method}`,
        `Контакт: ${data.contact}`,
        data.school ? `Сайт школы: ${data.school}` : null,
        data.pain ? `Боль: ${data.pain}` : null,
        '',
        (data.utm_source || data.utm_medium || data.utm_campaign || data.utm_content || data.fbclid || data.gclid) ? 'UTM:' : null,
